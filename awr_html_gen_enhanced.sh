#!/bin/bash
#===============================================================================
# AWR Parser v3.0 - HTML Dashboard Generator (CONFIG-AWARE)
# 
...
ARGUMENTS:
    csv_file        CSV file generated by awr_parser_v3.sh (REQUIRED)
...
EXAMPLES:
    ...
    $SCRIPT_NAME awr_analysis_*.csv dashboard.html

    # Use config for output directory
    source awr_config.sh
    $SCRIPT_NAME awr_analysis_*.csv

    # Even simpler - config sets input and output
    source awr_config.sh
    $SCRIPT_NAME
...
load_config() {
    ...
    if [[ -f "$config_file" ]]; then
        ...
        log_info "ℹ No config file found - using parameters only"
    fi
}
...
generate_html() {
    ...
    if [[ -z "$data" ]]; then
        log_error "CSV file is empty or has no data rows: $csv_file"
        return 1
    fi
    ...
    log_info "Extracted metrics: DB=$db_name, Instance=$instance_name, Version=$db_version"
    log_info "Health Status: $health_status | AAS=$aas | Buffer Hit=$buffer_hit%"
    ...
    <div class="container">
        ...
        <div class="cards">
            <div class="card health">
                ...
                <span class="status-badge healthy" id="health-badge">✓ Normal</span>
            </div>
            ...
            <div id="recommendations-container">
                <div class="recommendation-item">
                    ...
                    <div class="recommendation-detail">
                        ✓ Monitor In-Memory statistics (Adaptive Query Optimization)<br>
                        ✓ Review Automatic Indexing recommendations<br>
                        ✓ Enable SQL Plan Management for critical workloads<br>
                        ✓ Utilize Adaptive Dynamic Sampling
                    </div>
                </div>
                ...
                <div class="recommendation-item">
                    ...
                    <div class="recommendation-detail">
                        ✓ Analyze top SQL statements for optimization<br>
                        ✓ Review index usage and fragmentation<br>
                        ✓ Check for latch contention on high-traffic objects<br>
                        ✓ Validate I/O distribution across disks
                    </div>
                </div>
                ...
                <div class="recommendation-item">
                    ...
                    <div class="recommendation-detail">
                        ✓ Monitor redo log generation and application time<br>
                        ✓ Verify standby database sync status<br>
                        ✓ Review Real Application Clusters (RAC) GC efficiency<br>
...
HTMLEOF
    ...
    log_info "Dashboard ready - open in web browser to view"
...
case $# in
    0)
        # No arguments - should use config
        if [[ $CONFIG_LOADED -eq 0 ]]; then
            log_error "No arguments provided and no config file found"
            usage
        fi
        # Get CSV file from config or current directory
        CSV_FILE=$(ls awr_analysis_*.csv 2>/dev/null | head -1)
        if [[ -z "$CSV_FILE" ]]; then
            log_error "No CSV file found and no arguments provided"
            usage
        fi
        OUTPUT_HTML="${AWR_OUTPUT_PATH:-./}/index.html"
        ;;
    1)
        # Only CSV file provided - use config for output or default
        CSV_FILE="$1"
        OUTPUT_HTML="${AWR_OUTPUT_PATH:-./}/index.html"
        ;;
    2)
        # Both CSV and output HTML provided
        CSV_FILE="$1"
        OUTPUT_HTML="$2"
        ;;
    *)
...
log_info "CSV source: $CSV_FILE"
log_info "HTML output: $OUTPUT_HTML"
log_info ""

# Generate HTML
if generate_html "$CSV_FILE" "$OUTPUT_HTML"; then
    log_success "HTML Generation Complete!"
    exit 0
else
